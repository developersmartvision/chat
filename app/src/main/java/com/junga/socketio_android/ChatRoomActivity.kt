...
//            mSocket = IO.socket("http://203.210.87.95:8014/", options)
            options.path = "/masdidikchating/socket.io"
                mSocket = IO.socket("http://103.145.203.141", options)
    ...
    var onUpdateChat = Emitter.Listener {
        ...
        Log.d("onUpdateChat", chat.toString())
    ...
    private fun sendMessage() {
        ...
        val sendData = SendMessage(userName, content, roomName, noBlock, noMsgImage)
        val jsonData = gson.toJson(sendData)
        mSocket.emit("newMessage", jsonData)

        val message = Message(userName, content, roomName, noBlock, noMsgImage, MessageType.CHAT_MINE.index)
